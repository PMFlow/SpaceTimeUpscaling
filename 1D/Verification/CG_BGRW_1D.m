function [c,dmxx,dmvv,dmxv]=CG_BGRW_1D(c0,I,dx,dt,q,norm_q,D,mx,imx,a,ia)
                ...
                for i=1:2
                    ...
                    for m=1:length(mx)
                        if abs(x-imx(m))<=ia && abs(xd-imx(m))<ia   
                            ...
                            if norm_q==0
                                dmxv(m)=dmxv(m)+nj(i)*(x+xd)/2*dx*(xd-x)*dx/dt/(2*a);
                            else
                                dmxv(m)=dmxv(m)+njgrw(i)*(x+xd)/2*dx*(xd-x)*dx/dt/(2*a);
                            end
                        end
                    end
                end

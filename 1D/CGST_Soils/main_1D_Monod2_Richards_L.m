...
mx=(3/2*a:0.5*a:x2-3/2*a); imx=round(mx/dx);
fprintf('a = %.2f tau = %.2f\n',a,tau);
past=T/3;
t1=T/3;
Tolerance = 1e-5;
...
dI=(I-1)/10;
N=10^24; % = 1 mole
NI1=round(N/dI); NI2=round(1*N/(I-dI)); % NI = nr. of particles / lattice point
...
%% Flow solution
    for s=1:S
        ...
        nn(1)=n0E(1);
        if t<=t1
            nn(I)=n0E(I)+round(N*(-pBC(I)+0.2)*t/t1); 
        else
            nn(I)=round(N*0.2);
        end
    ...
    %% reaction
    [c1,c2,dmc1,dmc2,dmc1p,dmc2p,dmc1v,dmc2v]=CG_reaction_Monod2(c1t,c2t,dt,tht,Lc,I,mx,imx,a,ia);
    c10=c1; c20=c2;

...
D1=0.0001; % GRW
q=1*ones(1,I); % constant velocity
norm_q=norm(q);
if igrw==1
    Dfactor=2; % BGRW
...
NI=round(N/I); % 1 mol uniformly distributed on I points: (1/N)*(N/(x2-x1))=1 mol = mean concentration in the domain 
% ---> c=1/I mols & <1>/N=1/(2*a)*(mi/I); mi = nr. of points in [x-a x+a]
...
while t<=T
    ...
    %% Transport step
    if igrw==1
        [c1t,dmxx1,dmvv1,dmxv1]=CG_BGRW_1D(c10,I,dx,dt,q,norm_q,D1,mx,a); % BGRW transport solver
    else
        [c1t,dmxx1,dmvv1,dmxv1]=CG_GRW_1D(c10,I,dx,dt,q,D1,mx,a,d,stepU); % GRW transport solver
    end
    c1=c1t; c10=c1;   
    %% CGST Averages
    for k=1:length(mt)

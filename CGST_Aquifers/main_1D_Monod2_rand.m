%% CG averages over BGRW/GRW simulations for bimolecular (Monod) reactions; stochastic aerages
clear all; close all;
tic
initstate; state;
%% Grid Initialization
...
% %    1 mol uniformly distributed on subdomains:
    NI1=round(N/((I-1)/10)); NI2=round(N/(I-(I-1)/10)); 
    c10=zeros(1,I); c20=NI2*ones(1,I);
    c10(1:(I-1)/10)=NI1; c20(1:(I-1)/10)=0;
    c1BC=c10; c2BC=c20;
    %% Solution
    t=0;
    while t<=T
        ...
        for i=1:nspec
            switch i
                case 1
                    [c1t]=CG_BGRW_1D(c10,c1BC,I,dx,dt,q,D1); 
                case 2
                    [c2t]=CG_BGRW_1D(c20,c2BC,I,dx,dt,q,D1); 
            end
        end
        %% reaction
